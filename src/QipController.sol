...
contract QipController {
    ...
    modifier approvedContractOnly {
        require(msg.sender == owner || msg.sender == approved_contract, "Only the aproved smart contract can call this function");
        _;
    }
    ...
    constructor(address _ERC20_token, address _ERC721_token_1, address _ERC721_token_2, 
    address _ERC721_token_3, uint _price_1, uint _price_2, uint _price_3) {
    ...
    function enter_program(uint _rank) public {
        ...
        if (_rank == 3) {
            ...
            require(allowance_ >= status_badge_price_rank_3, "Allowance is not enough for an NFT");
            require(token.balanceOf(msg.sender) > status_badge_price_rank_3, "Balance is not enough");

            token.transferFrom(msg.sender, address(this), status_badge_price_rank_3);
            ...
            participants[msg.sender] = Participant(1, _tokenID_1, _tokenID_2, _tokenID_3, status_badge_price_rank_3, 0);
        } if (_rank == 2) {
            uint256 allowance_ = token.allowance(msg.sender, address(this));
            require(allowance_ >= status_badge_price_rank_2, "Allowance is not enough for an NFT");
            require(token.balanceOf(msg.sender) > status_badge_price_rank_2, "Balance is not enough");

            token.transferFrom(msg.sender, address(this), status_badge_price_rank_2);
            uint _tokenID_2 = status_badge_rank_2.mint(msg.sender);
            uint _tokenID_1 = status_badge_rank_1.mint(msg.sender);

            participants[msg.sender] = Participant(1, _tokenID_1, _tokenID_2, 0, status_badge_price_rank_2, 0);
        } if (_rank == 1) {
            uint256 allowance_ = token.allowance(msg.sender, address(this));
            require(allowance_ >= status_badge_price_rank_1, "Allowance is not enough for an NFT");
            require(token.balanceOf(msg.sender) > status_badge_price_rank_1, "Balance is not enough");

            token.transferFrom(msg.sender, address(this), status_badge_price_rank_1);
            uint _tokenID_1 = status_badge_rank_1.mint(msg.sender);

            participants[msg.sender] = Participant(1, _tokenID_1, 0, 0, status_badge_price_rank_1, 0);
        }
    }
    ...
    function leaveProgram() public {
        require(participants[msg.sender].rank != 0, "You are not in the program");

        if (participants[msg.sender].rank == 3) {
            require(status_badge_rank_3.ownerOf(participants[msg.sender].tokenID_rank_3) == msg.sender, "You do not own this NFT 3");
            require(status_badge_rank_2.ownerOf(participants[msg.sender].tokenID_rank_2) == msg.sender, "You do not own this NFT 2");
            require(status_badge_rank_1.ownerOf(participants[msg.sender].tokenID_rank_1) == msg.sender, "You do not own this NFT 1");
    ...
    function reward_nft_purchase(address _invite_creator) approvedContractOnly public {
        participants[_invite_creator].pool = participants[_invite_creator].pool + nft_purchase_amount;
        emit RewardedNftPurchase(_invite_creator);
    }
    
    function reward_cardpack_purchase(address _invite_creator) approvedContractOnly public {
        participants[_invite_creator].pool = participants[_invite_creator].pool + cardpack_purchase_amount;
        emit RewardedCardpackPurchase(_invite_creator);
    }

    function reward_nft_rank(address _invite_creator) approvedContractOnly public {
        participants[_invite_creator].pool = participants[_invite_creator].pool + nft_rank_amount;
    ...
    function upgrade(bool _UseRewards) public {
        ...
        if (participants[msg.sender].rank == 1 && _UseRewards == true) {
            ...
            uint _amountNeeded = status_badge_price_rank_2 - participants[msg.sender].entryPrice;
            if (_balance >= _amountNeeded) {
                ...
                require(token.allowance(msg.sender, address(this)) +  _balance >= _amountNeeded,
                "Your allowance isn't enough");
                require(token.balanceOf(msg.sender) + _balance >= _amountNeeded, "Your balance is too low");  

                participants[msg.sender].pool = 0;
                participants[msg.sender].entryPrice = status_badge_price_rank_2; 

                token.transferFrom(msg.sender, address(this), _amountNeeded - _balance);
                uint _tokenID_2 = status_badge_rank_2.mint(msg.sender);
                participants[msg.sender].tokenID_rank_2 = _tokenID_2;
            ...
            uint _balance = participants[msg.sender].pool;
            uint _amountNeeded = status_badge_price_rank_3 - participants[msg.sender].entryPrice;
            if (_balance >= _amountNeeded) {
                participants[msg.sender].pool = _amountNeeded - _balance;
            ...
            uint _amountNeeded = status_badge_price_rank_2 - participants[msg.sender].entryPrice;
            require(token.allowance(msg.sender, address(this)) >= _amountNeeded,
                "Your allowance isn't enough");
            require(token.balanceOf(msg.sender) >= _amountNeeded, "Your balance is too low");  
    ...
    function adjust_entry_price_rank_1(uint256 _newPrice) public approvedContractOnly {
        status_badge_price_rank_1 = _newPrice * token.decimals();
        emit NewPrice(status_badge_price_rank_1);
    }

    function adjust_entry_price_rank_2(uint256 _newPrice) public approvedContractOnly {
        status_badge_price_rank_2 = _newPrice * token.decimals();
        emit NewPrice(status_badge_price_rank_2);
    }

    function adjust_entry_price_rank_3(uint256 _newPrice) public approvedContractOnly {
        status_badge_price_rank_3 = _newPrice * token.decimals();
        emit NewPrice(status_badge_price_rank_3);
    }

    function adjust_reward_nft_purchase(uint _newRewardAmount) public approvedContractOnly {
        nft_purchase_amount = _newRewardAmount;
    }

    function adjust_reward_cardpack_purchase(uint _newRewardAmount) public approvedContractOnly {
        cardpack_purchase_amount = _newRewardAmount;
    }

    function adjust_reward_nft_rank(uint _newRewardAmount) public approvedContractOnly {
        nft_rank_amount = _newRewardAmount;
    }

    function changeToken(address _newToken, uint _newPrice_1, uint _newPrice_2, uint _newPrice_3) public approvedContractOnly {
